...
braf_train <- selectdata_train_all[,-1]
braf_test <- selectdata_test_all[,-1]
colnames(braf_train)[1] <- "TYPE"
colnames(braf_test)[1] <- "TYPE"
train1=braf_train
test1=braf_test
...
set.seed(123)
mtry_fit=train(TYPE_A~.,data=train1, method = "rf", metric="ROC",tuneGrid=tuneGrid,trControl=trControl)

tuneGrid=expand.grid(mtry=c(3))
store_maxtrees=list()
for (ntree in c(50,100,150,200,250,300,350,400,450,500,550,
                600,650,700,750,800)){set.seed(123)
  rf_maxtrees=train(TYPE_A~.,data=train1, method = "rf", metric="ROC",tuneGrid=tuneGrid,trControl=trControl,ntree=ntree)
  key=toString(ntree)
...
set.seed(4)
modelfit1=train(TYPE~.,data=train1, method = "rf", metric="ROC",tuneGrid=expand.grid(mtry=c(3)),ntree=500,
...
roc_train1= roc(braf_train[,1],predictions1_prob_train[,1])
roc_test1= roc(braf_test[,1],predictions1_prob[,1])

######
#install.packages('modEvA')
library(modEvA)
aupr=AUC(obs=braf_train[,1],pred=predictions1_prob_train[,1],curve = "PR", simplif=TRUE, main = "TRAIN PR curve",plot.values=F)
aupr=AUC(obs=braf_test[,1],pred=predictions1_prob[,1],curve = "PR", simplif=TRUE, main = "TEST PR curve",plot.values=F)

######

pred_train1=prediction(predictions1_prob_train[,1],braf_train[,1])
pred_test1=prediction(predictions1_prob[,1],braf_test[,1])
